    ...
    <section>
      <div className="container space-y-12">
        <div className="space-y-6">
          <Link
            href="/tours"
            className="inline-flex items-center gap-2 text-sm font-semibold text-brand-dark hover:text-brand dark:text-brand-light dark:hover:text-brand transition-colors"
          >
            ← Back to tours
          </Link>
          <div className="grid gap-10 lg:grid-cols-[1.1fr,0.9fr] lg:items-start">
            <div className="space-y-6">
              ...
              <h1 className="text-4xl font-display font-semibold   dark:text-slate-100">
                {tour.title}
              </h1>
              ...
              <div className="grid gap-4 md:grid-cols-3">
                <div className="card space-y-1 text-sm   dark:text-slate-300">
                ...
                <div className="card space-y-1 text-sm   dark:text-slate-300">
                  ...
                  <p className="text-base font-semibold text-brand-dark dark:text-brand-light">
                    {tour.price}
                  </p>
                </div>
              </div>
              ...
              <div className="space-y-4">
                ...
                <ul className="grid gap-3 text-sm   dark:text-slate-300 md:grid-cols-2">
                  {tour.highlights.map((highlight) => (
                    <li
                      key={highlight}
                      className="rounded-2xl border border-slate-200/80 bg-white/70 p-4 dark:border-slate-700/80 dark:bg-slate-800/70"
                    >
                      {highlight}
                    </li>
                  ))}
                  ...
                  {tour.itinerary.map((day) => (
                    <div key={day.day} className="card">
                      <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                        <span className="text-xs font-semibold uppercase tracking-wide text-brand-dark dark:text-brand-light">
                          {day.day}
                        </span>
                      ...
                      <p className="mt-3 text-sm   dark:text-slate-300">
                        {day.details}
                      </p>
                    </div>
                  ))}
