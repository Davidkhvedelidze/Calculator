  ...
  return (
    <article className="card overflow-hidden">
      <div
        className={
          layout === "horizontal"
            ? "grid gap-6 lg:grid-cols-[1.3fr,1fr]"
            : "space-y-6"
        }
      >
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="flex flex-wrap gap-3 text-sm  ">
            {tour.highlights.slice(0, 3).map((highlight) => (
              <span
                key={highlight}
                className="rounded-full bg-slate-100 dark:bg-slate-700 px-3 py-1"
              >
                {highlight}
              </span>
            ))}
              ...
              <button
                ...
                className={`inline-flex h-10 w-10 items-center justify-center rounded-full border transition ${
                  isFavorite
                    ...
                    : "border-slate-200 text-slate-500 hover:border-brand hover:text-brand-dark"
                }`}
                aria-pressed={isFavorite}
                aria-label={
                  isFavorite ? "Remove from saved tours" : "Save tour"
                }
              >
                <svg
                  width="20"
